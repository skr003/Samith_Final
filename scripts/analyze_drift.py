...
def analyze():
    ...
    for res in data.get("resources", []):
        ...
        if "Microsoft.Storage/storageAccounts" in res.get("type", ""):
            ...
            passed = val == "Disabled"
            record_check(results, rid, "1,7", "Storage: Public network access disabled",
                         passed, f"publicNetworkAccess={val}")

            # PCI DSS Req 3: Encryption at rest enabled
            val = acc.get("encryption", {}).get("services")
            passed = bool(val)
            record_check(results, rid, "3", "Storage: Encryption at rest enabled",
                         passed, f"services={val}")

            # PCI DSS Req 10: Logging enabled
            diag = acc.get("diagnostics_profile", {}).get("boot_diagnostics", {}).get("enabled")
            passed = diag is True
            record_check(results, rid, "10", "Storage: Boot diagnostics enabled",
                         passed, f"boot_diagnostics={diag}")
        ...
        if "Microsoft.Compute/virtualMachines" in res.get("type", ""):
            ...
            passed = bool(key)
            record_check(results, rid, "2", "VM: Secure SSH key configured",
                         passed, f"ssh_key_present={bool(key)}")

            # PCI DSS Req 10: Diagnostic logging
            diag = vm.get("diagnostics_profile", {}).get("boot_diagnostics", {}).get("enabled")
            passed = diag is True
            record_check(results, rid, "10", "VM: Boot diagnostics enabled",
                         passed, f"boot_diagnostics={diag}")
        ...
        if "Microsoft.Authorization/roleAssignments" in res.get("type", ""):
            ...
            record_check(results, rid, "7", "IAM: Least privilege enforced",
                         passed, f"role={role}")

            # PCI DSS Req 8: MFA required
            mfa = iam.get("mfaEnabled")
            passed = mfa is True
            record_check(results, rid, "8", "IAM: MFA enabled",
                         passed, f"mfaEnabled={mfa}")
        ...
        if "Microsoft.DBfor" in res.get("type", ""):
            ...
            passed = enc == "Enabled"
            record_check(results, rid, "3", "DB: SSL enforcement enabled",
                         passed, f"sslEnforcement={enc}")

            # PCI DSS Req 7: Public network disabled
            pub = db.get("publicNetworkAccess")
            passed = pub == "Disabled"
            record_check(results, rid, "7", "DB: Public network access disabled",
                         passed, f"publicNetworkAccess={pub}")
